...
import type { Menuparams } from "antd";
import { useTranslations } from "next-intl";
import { ComponentSelectCity } from "@/features/ComponentSelectCity";


export default function ComponentHeader({ params }: { params: any }) {
  const mobile = { flex: "100%" };
  const tablet = { flex: "50%" };

  const t = useTranslations();

  const accountItems: Menuparams["items"] = [
    {
      key: "0",
      label: <Link href="/account">{t("akkaunt")}</Link>,
    },
    {
      key: "1",
      label: (
    ...
    {
      key: "2",
      label: <span>Русский{/* <LangSwitcher /> */}</span>,
    },
  ];
